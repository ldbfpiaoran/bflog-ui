...
var Head = /** @class */ (function (_super) {
    ...
    Head.prototype.render = function () {
        ...
        return (_J$X_("thead", { ref: this.domRef, className: cx('Table-thead') }, this.thColumns.map(function (data, index) {
            return (_J$X_("tr", { key: 'th-cell-' + index },
                draggable && index === 0 ? (_J$X_(Cell["default"], { key: "drag-".concat(index), wrapperComponent: "th", rowSpan: _this.thColumns.length, className: cx('Table-dragCell'), col: "drag", classnames: cx, classPrefix: classPrefix })) : null,
                !draggable && selectable && index === 0 ? (_J$X_(Cell["default"], { key: "select-".concat(index), wrapperComponent: "th", rowSpan: _this.thColumns.length, fixed: rowSelectionFixed ? 'left' : '', className: cx('Table-checkCell'), col: "select", classnames: cx, classPrefix: classPrefix }, rowSelectionType !== 'radio'
                    ? [
                ...
                data.map(function (item, i) {
                    ...
                    if (item.filterDropdown) {
                        ...
                        filter = (_J$X_(HeadCellFilter["default"], { column: item, popOverContainer: popOverContainer, onFilter: onFilter }));
                    }
                    // th的最后一行才可调整列宽
                    // 分组情况下 最后一行才和列配置个数对应
                    ...
                    return (_J$X_(Cell["default"], { key: "cell-".concat(i), wrapperComponent: "th", rowSpan: item.rowSpan, colSpan: item.colSpan, classnames: cx, classPrefix: classPrefix, fixed: item.fixed === true ? 'left' : item.fixed, className: cx({
