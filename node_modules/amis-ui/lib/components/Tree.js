...
var TreeSelector = /** @class */ (function (_super) {
    ...
    TreeSelector.prototype.renderInput = function (prfix) {
        ...
        return (_J$X_("div", { className: cx('Tree-itemLabel', {
            ...
            _J$X_("div", { className: cx('Tree-itemInput', {
                ...
                prfix,
                _J$X_("input", { onChange: this.handleInputChange, value: inputValue, placeholder: __('placeholder.enter') }),
                _J$X_("a", { "data-tooltip": __('cancel'), onClick: this.handleCancel },
                    _J$X_(icons.Icon, { icon: "close", className: "icon" })),
                _J$X_("a", { "data-tooltip": __('confirm'), onClick: this.handleConfirm },
    ...
    TreeSelector.prototype.renderItem = function (_a) {
        ...
        var checkbox = multiple ? (_J$X_(Checkbox["default"], { size: "sm", disabled: disabled, checked: checked || partial, partial: partial, onChange: this.handleCheck.bind(this, item, !checked) })) : showRadio ? (_J$X_(Checkbox["default"], { size: "sm", disabled: disabled, checked: checked, onChange: this.handleSelect.bind(this, item) })) : null;
        ...
        if (isEditing && editingItem === item) {
            ...
            body = this.renderInput(_J$X_("span", { className: cx('Tree-itemArrowPlaceholder') }));
        }
        else {
            body = (_J$X_("div", { className: cx('Tree-itemLabel', {
                ...
                draggable && (_J$X_("a", { className: cx('Tree-itemDrager drag-bar') },
                    _J$X_(icons.Icon, { icon: "drag-bar", className: "icon" }))),
                item.loading ? (_J$X_(Spinner["default"], { size: "sm", show: true, icon: "reload", spinnerClassName: cx('Tree-spinner'), loadingConfig: loadingConfig })) : !isLeaf || (item[deferField] && !item.loaded) ? (_J$X_("div", { onClick: function () { return _this.toggleUnfolded(item); }, className: cx('Tree-itemArrow', {
                ...
                checkbox,
                _J$X_("div", { className: cx('Tree-itemLabel-item', { 'is-mobile': mobileUI }) },
                    showIcon ? (_J$X_("i", { className: cx("Tree-itemIcon ".concat(Array.isArray(item.children) && item.children.length
                        ...
                        } }, iconValue ? (icons.getIcon(iconValue) ? (_J$X_(icons.Icon, { icon: iconValue, className: "icon" })) : React__default["default"].isValidElement(iconValue) ? (iconValue) : (_J$X_("i", { className: iconValue }))) : null)) : null,
                    _J$X_("span", { className: cx('Tree-itemText'), onClick: function () {
                            return !disabled &&
                                (multiple
                                    ? _this.handleCheck(item, !checked)
                                    : _this.handleSelect(item));
                    ...
                    !disabled &&
                        !isAdding &&
                        !isEditing &&
                        !(item[deferField] && !item.loaded) ? (_J$X_("div", { className: cx('Tree-item-icons') },
                        creatable && amisCore.hasAbility(item, 'creatable') ? (_J$X_(TooltipWrapper["default"], { placement: 'bottom', tooltip: __(createTip), trigger: 'hover', tooltipTheme: "dark" },
                            _J$X_("a", { onClick: this.handleAdd.bind(this, item) },
                        ...
                        removable && amisCore.hasAbility(item, 'removable') ? (_J$X_(TooltipWrapper["default"], { placement: 'bottom', tooltip: __(removeTip), trigger: 'hover', tooltipTheme: "dark" },
                            _J$X_("a", { onClick: this.handleRemove.bind(this, item) },
                        ...
                        editable && amisCore.hasAbility(item, 'editable') ? (_J$X_(TooltipWrapper["default"], { placement: 'bottom', tooltip: __(editTip), trigger: 'hover', tooltipTheme: "dark" },
                            _J$X_("a", { onClick: this.handleEdit.bind(this, item) },
                                _J$X_(icons.Icon, { icon: "new-edit", className: "icon" })))) : null)) : null)));
        }
        return (_J$X_("li", { key: "".concat(item[valueField || 'value'], "-").concat(index), className: cx("Tree-item ".concat(itemClassName || ''), {
                'Tree-item--isLeaf': isLeaf,
                'is-child': this.relations.get(item)
            }), style: tslib.__assign(tslib.__assign({}, style), { paddingLeft: "calc(".concat(level, " * var(--Tree-indent))") }) }, body));
    };
    ...
    TreeSelector.prototype.renderList = function (list, value) {
        ...
        if (virtualThreshold && list.length > virtualThreshold) {
            return (_J$X_(index["default"], { height: list.length > 8 ? 266 : list.length * itemHeight, itemCount: list.length, prefix: this.renderCheckAll(), itemSize: itemHeight, 
    ...
    TreeSelector.prototype.render = function () {
        ...
        if (creatable && rootCreatable !== false && hideRoot) {
            addBtn = (_J$X_("a", { className: cx('Tree-addTopBtn', {
        ...
        return (_J$X_("div", { className: cx("Tree ".concat(className || ''), {
            ...
            (flattenedOptions && flattenedOptions.length) ||
                ...
                this.renderList(flattenedOptions, value))) : (_J$X_("li", { className: cx('Tree-rootItem', {
                    'is-checked': !value || !value.length
                }) },
                _J$X_("div", { className: cx('Tree-itemLabel') },
                    _J$X_("span", { className: cx('Tree-itemText'), onClick: this.clearSelect },
                    ...
                    !disabled &&
                        ...
                        rootCreatable !== false &&
                        !isAdding &&
                        !isEditing ? (_J$X_("div", { className: cx('Tree-item-icons') }, creatable ? (_J$X_("a", { onClick: this.handleAdd.bind(this, null), "data-tooltip": rootCreateTip, "data-position": "left" },
                        _J$X_(icons.Icon, { icon: "plus", className: "icon" }))) : null)) : null),
                _J$X_("ul", { className: cx('Tree-sublist') },
                    isAdding && !addingParent ? (_J$X_("li", { className: cx('Tree-item') }, this.renderInput())) : null,
                    this.renderList(flattenedOptions, value)))))) : (_J$X_("div", { className: cx('Tree-placeholder') }, placeholder)),
