...
var Table = /** @class */ (function (_super) {
    ...
    Table.syncRows = function (store, props, prevProps) {
        ...
        if (Array.isArray(value) &&
            (!prevProps || !isEqual__default["default"](prevProps.value || prevProps.items, value))) {
            updateRows = true;
            rows = value;
        }
    ...
    Table.prototype.renderHeadCell = function (column, props) {
        ...
        var _d = tslib.__read(store.getStickyStyles(column, store.filteredColumns), 2), stickyStyle = _d[0], stickyClassName = _d[1];
        Object.assign(style, stickyStyle);
        var resizeLine = (_J$X_("div", { className: cx('Table-content-colDragLine'), key: "resize-".concat(column.id), "data-index": column.index, onMouseDown: this.handleColResizeMouseDown }));
        // th 里面不应该设置
        ...
        if (column.type === '__checkme') {
            return (_J$X_("th", tslib.__assign({}, props, { style: style, className: cx(column.pristine.className, stickyClassName) }),
                store.rows.length && store.multiple ? (_J$X_(amisUi.Checkbox, { classPrefix: ns, partial: store.someChecked && !store.allChecked, checked: store.someChecked, disabled: store.isSelectionThresholdReached, onChange: this.handleCheckAll })) : ('\u00A0'),
    ...
    Table.prototype.renderFooter = function () {
        ...
        var toolbarNode = actions || child ? (_J$X_("div", { className: cx('Table-toolbar Table-footToolbar', toolbarClassName, footerToolbarClassName), key: "footer-toolbar" },
            actions,
            child)) : null;
        var footerNode = footer && (!Array.isArray(footer) || footer.length) ? (_J$X_("div", { className: cx('Table-footer', footerClassName), key: "footer" }, render('footer', footer, {
            data: store.getData(data)
        }))) : null;
